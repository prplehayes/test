        ...
        <thead>
           <tr>
               <th width="80px"><input type="checkbox" name="chkallp" id="chkallp" value="1">
              &nbsp;All</th>
        ...
        <tbody>
            ...
            <tr>
               <td>
			<input type="checkbox" name="clid[]" class="chkp" value="<?php echo $claim->id?>">
			</td>
...
$("#allpendingreview .pagination a").on('click', function(e){

        var href = $(this).attr('href');
		
         $.ajax({url:href,type: "POST",data:$(".frmfilterpn").serialize(), success: function(result){
	$("#allpendingreview").html(result);         
        }});
            //$('#allpendingreview').load($(this).attr('href'));
       
        return false;
    });
$("#allpendingreview .sort a").on('click', function(){
