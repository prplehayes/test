        ...
        <tbody>
            ...
            <tr>
			   ...
			   echo $payment[$claim->id]?></td>
                <td class="actions">
                 <a href="<?php echo $this->request->webroot?>claim/view/<?php echo $claim->id?>" target="_blank">Pending</a>
...
$("#appealclaimlist .pagination a").on('click', function(e){

        var href = $(this).attr('href');
		
       /* $.ajax({url:href,type: "POST",data:$(".frmfilterpaid").serialize(), success: function(result){
			$("#appealclaimlist").html(result);         
        }});*/
           $('#appealclaimlist').load($(this).attr('href'));
       
        return false;
    });
$("#appealclaimlist .sort a").on('click', function(){
