        ...
        <tbody>
            ...
            <tr>
			   ...
			   echo $claimlist['pay_amount'][$claim->id]?></td>
                <td class="actions">
                 <a href="<?php echo $this->request->webroot?>claim/claimappealbillreview/<?php echo $claim->id?>" target="_blank"><?php echo $claimlist['status'][$claim->id];?></a>
...
$("#allappealclaimbill .pagination a").on('click', function(e){

        var href = $(this).attr('href');
		
       /* $.ajax({url:href,type: "POST",data:$(".frmfilterpaid").serialize(), success: function(result){
			$("#allappealclaimbill").html(result);         
        }});*/
           $('#allappealclaimbill').load($(this).attr('href'));
       
        return false;
    });
$("#allappealclaimbill .sort a").on('click', function(){
