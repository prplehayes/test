        ...
        <tbody>
            ...
            <tr>
			   ...
			   echo $claimlist['pay_amount'][$claim->id]?></td>
                <td class="actions">
                 <a href="<?php echo $this->request->webroot?>claim/payappeal/<?php echo $claim->id?>" target="_blank">Pay</a>
				  |
				  <a href="<?php echo $this->request->webroot?>claim/view/<?php echo $claim->id?>">View</a> 
...
$("#allappealclaimpaid .pagination a").on('click', function(e){

        var href = $(this).attr('href');
		
       /* $.ajax({url:href,type: "POST",data:$(".frmfilterpaid").serialize(), success: function(result){
			$("#allappealclaimpaid").html(result);         
        }});*/
           $('#allappealclaimpaid').load($(this).attr('href'));
       
        return false;
    });
$("#allappealclaimpaid .sort a").on('click', function(){
