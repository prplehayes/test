        ...
        <tbody>
            ...
            <tr>
			   ...
			   echo $claimlist['pay_amount'][$claim->id]?></td>
                <td class="actions">
                 <a href="<?php echo $this->request->webroot?>claim/claimappealreview/<?php echo $claim->id?>" target="_blank">Pending Review</a>
...
$("#allappealclaim .pagination a").on('click', function(e){

        var href = $(this).attr('href');
		
       /* $.ajax({url:href,type: "POST",data:$(".frmfilterpaid").serialize(), success: function(result){
			$("#allappealclaim").html(result);         
        }});*/
           $('#allappealclaim').load($(this).attr('href'));
       
        return false;
    });
$("#allappealclaim .sort a").on('click', function(){
