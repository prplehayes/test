        ...
        <tbody>
            ...
            <tr>
			   ...
			   echo $claimlist['pay_amount'][$claim->id]?></td>
                <td class="actions">
                 <a href="<?php echo $this->request->webroot?>claim/view/<?php echo $claim->id?>" target="_blank">Paid</a>
				 </td>
            </tr>
            <?php endforeach; ?>
...
$("#dentistappealclaimpaidlist .pagination a").on('click', function(e){

        var href = $(this).attr('href');
		
       /* $.ajax({url:href,type: "POST",data:$(".frmfilterpaid").serialize(), success: function(result){
			$("#dentistappealclaimpaidlist").html(result);         
        }});*/
           $('#dentistappealclaimpaidlist').load($(this).attr('href'));
       
        return false;
    });
$("#dentistappealclaimpaidlist .sort a").on('click', function(){
