...
<div class="billview1" style="float:left;padding:10px;width:100%;">
    <h3><?= h($patient->first_name) ?>- Bill Summary</h3>
	<div class="span12 nomarginleft">
		...
		<div><strong><?= __('CPT Codes') ?></strong><br><br>
			...
			?>
                 <div class="cptbox">
				<div class="inlinebox"><?= h($cptCode->code) ?> < <?= h($cptCode->description) ?></div>
		...
		<div><strong><?= __('ICD-10 Codes') ?></strong><br><br>
            ...
            <?php foreach ($icd10Codes as $icd10Code): ?>
                <div class="icd10box">
				<div class="inlinebox"><?= h($icd10Code->code) ?> - <?= h($icd10Code->description) ?></div></div>
            <?php endforeach; ?>
        <?php endif; ?>
			</div>
		</div>
